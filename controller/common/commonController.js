...
module.exports = {
					...
					if (findProfile) {
						switch (type) {
							case '1':
								...
								if (!(student_university_email && req.files.upload_profile_photo && first_name && last_name && dob && mobile_no && street_name && street_address1 && city && state && zipcode && ssn && emergency_contact_no && gender && vehicle_registration_expiration_date && insurance_expiration_date && state_abbr && issue_date && class_description && license_expiration_date && class_code && previous_license)) {
									return errorResponse(res, 'Required All Fields')
								} else {
									var newvalues = {
										$set: {
											...
											profile_photo: req.files.upload_profile_photo[0].destination + req.files.upload_profile_photo[0].filename,
											vehicle_registration_expiration_date: vehicle_registration_expiration_date,
								...
								break;
							case '2':
								if (!(student_university_email && req.files.upload_profile_photo && first_name && last_name && dob && mobile_no && street_name && street_address1 && city && state && zipcode && ssn && emergency_contact_no && gender)) {
									return errorResponse(res, 'Required All Fields')
								} else {
									var newvalues = {
										$set: {
							...
							switch (type) {
								case '1':
									if (!(student_university_email && req.files.upload_profile_photo && first_name && last_name && dob && mobile_no && street_name && street_address1 && city && state && zipcode && ssn && emergency_contact_no && gender && vehicle_registration_expiration_date && insurance_expiration_date && state_abbr && issue_date && class_description && license_expiration_date && class_code && previous_license)) {
										...
										let profileInf = new profileDB();
										profileInf.profile_id = profile_id,
											profileInf.fullname = first_name + ' ' + middle_name + '' + last_name
										profileInf.uuid = '',
											profileInf.first_name = first_name,
											profileInf.middle_name = middle_name | '',
											...
											profileInf.street_address1 = street_address1 || '',
											profileInf.street_address2 = street_address2 || '',
											...
											profileInf.vehicle_registration_expiration_date= vehicle_registration_expiration_date,
											profileInf.insurance_expiration_date= insurance_expiration_date,
											profileInf.state_abbr= state_abbr,
											profileInf.issue_date= issue_date,
											profileInf.class_description= class_description,
											profileInf.license_expiration_date= license_expiration_date,
											profileInf.class_code= class_code,
											profileInf.previous_license= previous_license,
											profileInf.rating = 0,
											profileInf.backgroundPayStatus = 0,
											profileInf.created_date = Date.now()
								...
								case '2':
									if (!(student_university_email && req.files.upload_profile_photo && first_name && last_name && dob && mobile_no && street_name && street_address1 && city && state && zipcode && ssn && emergency_contact_no && gender)) {
										...
										profileInfor.profile_id = profile_id,
											profileInfor.uuid = '',
											profileInfor.fullname = first_name + ' ' + middle_name + '' + last_name,
											profileInfor.first_name = first_name,
											profileInfor.middle_name = middle_name || '',
											...
											profileInfor.street_address1 = street_address1 || '',
											profileInfor.street_address2 = street_address2 || '',
	...
	getUserProfileData: async function (req, res) {
		try {
			...
			if (profile_id) {
				...
				paymentData = await paymentMethodDB.findOne({ user_id: profile_id, payment_status: '1' });
				var paymentMessage;
				if (paymentData) {
	...
	//                 paymentData = await paymentMethodDB.findOne({ user_id: profile_id });
	...
	checkUserProfileExist: async function (req, res) {
		...
		try {
			const profile_id = await req.user.id;
			// console.log("profile_id --------->",req.user.id);
			if (profile_id) {
				if (req.body.type == '' || req.body.type == null || req.body.type == undefined) {
					return validationError(res, 'type Field is required')
				} else {
					...
					const emailVerified = await signupDB.findOne({
						_id: profile_id
					});
					// console.log("emailVerified", emailVerified)
					// console.log("userProfile",userProfile);
					if (emailVerified.email_verified == 1) {
						if (userProfile == '' || userProfile == null || userProfile.length <= 0) {
							return successWithData(res, "Profile is incomplete yet")
						} else if (paymentData == null || paymentData.payment_status != "1") {
							return successWithData(res, "Payment is incomplete")
						} else if (userProfile.backgroundPayStatus == 0) {
							return successWithData(res, "Background Payment is incomplete yet")
						} else if (userProfile.uuid == '' || userProfile.uuid == null || userProfile.uuid == undefined) {
							return success(res, "Background Verification is incomplete yet")
						...
						return validationError(res, 'Email Not Verified')
					}
	...
	editProfile: async function (req, res) {
		try {
			...
			if (profile_id) {
				...
				if (!(university_name, student_id, city, state, zipcode, mobile_no, student_university_email, street_address1, gender /*payment_info upload_profile_photo*/)) {
					...
					if (update1) {
						const update = await profileDB.updateOne({
						...
						},
							{
								...
								last_name: last_name,
								student_university_email: student_university_email.toLowerCase(),
								university_name: university_name || update1.university_name,
								student_id: student_id || update1.student_id,
								university_address: university_address || update1.university_address,
								mobile_no: mobile_no,
								dob: dob,
								house_number: house_number || '',
								street_name: street_name,
								...
								ssn: ssn,
								gender: gender,
								car_model: car_model || update1.car_model,
								vehicle_color: car_color || update1.vehicle_color,
								//vehicle_plate_number: vehicle_plate_number,
								destination_contact_number: destination_contact_number || update1.destination_contact_number,
								type: type,
								gender_preferences: gender_preferences || update1.gender_preferences,
								rider_preference: rider_preference || update1.rider_preference,
								phone_code: phone_code || update1.phone_code,
								phone_no: phone_no || update1.phone_no,
								emergency_contact_no: emergency_contact_no || update1.emergency_contact_no,
								// profile_photo: req.files.upload_profile_photo[0].destination + req.files.upload_profile_photo[0].filename,
								updated_date: Date.now(),
							}, {
							new: true
	...
	savePaymentStatus: async function (req, res) {
		try {
			const profile_id = await req.user.id;
			if (profile_id) {
				console.log("profile_id--------->", profile_id);
				...
				console.log("---------------------->", req.body);
				if (!(paymentIntentID && payable_amount && payment_status && customer_id)) {
					return validationError(res, 'All data is mandatory')
				} else {
					// const userProfile = await profileDB.findOne({
					//     profile_id: profile_id,
					//     type: req.body.type
					...
					//console.log('doc',doc)
					payinfo.user_id = profile_id,
						payinfo.payment_id = paymentIntentID,
						payinfo.payable_amount = payable_amount,
						payinfo.payment_status = payment_status,
						payinfo.customer_id = customer_id,
						//payinfo.payment_type=paymentIntent.payment_method_types
						payinfo.save(async (err, doc) => {
							if (err) {
								console.log('err', err)
								return errorResponse(res, 'Data Not Found');
							} else {
								const findUser = await profileDB.findOne({ profile_id: profile_id })
								console.log("findUser----------->", findUser, findUser.dob)
								const authUserData = {
									"firstName": findUser.first_name,
									"middleName": findUser.middle_name,
									"lastName": findUser.last_name,
								...
								if (findUser.type == 1) {
									preferredWorkflowID = "GN4jWIXVDK"
								} else {
									preferredWorkflowID = "sxkJWbT5ox"
								}
								...
								try {
									const response1 = await axios.post('https://api-v3.authenticating.com/user/create', authUserData, axiosConfig);
									console.log("response1 ---------->", response1)
									const updatestatus = await profileDB.updateOne({
										_id: findUser._id
									}, {
										uuid: response1.data.userAccessCode,
										backgroundPayStatus: 1
									...
									if (updatestatus) {
										console.log(res, 'Successfully Deleted', updatestatus);
									} else {
									...
									if (updatedUuid) {
										...
										const response2 = await axios.post('https://api-v3.authenticating.com/user/jwt', {
											...
											redirectURL: 'https://verify.authenticating.com?token=',
										}, axiosConfig);
									...
									const data = {
										...
										payment: 1,
										workflowID: preferredWorkflowID,
										AuthFailureMsg: err.response.data['errorMessage']
									}
									console.log(err);
									return successWithData(res, 'Payment Info added Successfully', data);
								}

								// return successWithData(res, 'Payment Info added Successfully', doc);
							}
						})
				}
			}
	...
	getAllStates: async function (req, res) {
		try {
			return successWithData(res, 'Success', require('../../usa_states.json'));
		} catch (err) {
			console.log(err);
		}
	},

	getAllInstitutes: async function (req, res) {
		try {
			return successWithData(res, 'Success', require('../../us_institutions.json'));
		} catch (err) {
			console.log(err);
		}
	},

	getTotalData: async function (req, res) {
		try {
			const driverCount = await signupDB.countDocuments({ role_id: 1 });
			const riderCount = await signupDB.countDocuments({ role_id: 2 });
			const totalCount = await signupDB.countDocuments({ $or: [{ role_id: 1 }, { role_id: 2 }] });
			const tripsCount = await tripDB.countDocuments();
	...
	getUsersByMonths: async function (req, res) {
		try {
			const data = await signupDB.find().exec();
			...
			return successWithData(res, 'Success', formattedMonthYearCounts)
		} catch (error) {
			throw new Error(`Error counting users: ${error.message}`);
		}
	},

	getPieChartsDetails: async function (req, res) {
		try {
			const currentDate = new Date();
			const startOfMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1);
			const endOfMonth = new Date(currentDate.getFullYear(), currentDate.getMonth(), 0);
			const totalUser = await signupDB.countDocuments({ $or: [{ role_id: 1 }, { role_id: 2 }] });
			const currentMonth = await signupDB.countDocuments({ created_date: { $gte: startOfMonth, $lte: endOfMonth } }).exec();
			const totalDrivers = await signupDB.countDocuments({ role_id: 1 });
			const currentMonthD = await signupDB.countDocuments({ $and: [{ role_id: 1 }, { created_date: { $gte: startOfMonth, $lte: endOfMonth } }] }).exec();
			const totalRiders = await signupDB.countDocuments({ role_id: 2 });
			const currentMonthR = await signupDB.countDocuments({ $and: [{ role_id: 2 }, { created_date: { $gte: startOfMonth, $lte: endOfMonth } }] }).exec();
			return successWithData(res, 'Success', {
				user: {
					total: totalUser,
